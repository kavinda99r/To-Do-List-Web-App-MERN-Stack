...
const TodoList = () => {
  ...
  const handleAddTodo = async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
      const response = await axios.post('http://localhost:5001/api/todos', { task }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      console.error(err);
      toast.error('Error adding task', { style: { backgroundColor: 'red', color: 'white' } }); // Red toast for error
    }
  };
  ...
  const handleToggleComplete = async (id, currentStatus) => {
    ...
    try {
      ...
      console.error(err);
      toast.error('Error updating task', { style: { backgroundColor: 'red', color: 'white' } }); // Red toast for error
    }
  };
  ...
  const handleDeleteTodo = async (id) => {
    ...
    try {
      ...
      toast.error('Error deleting task', { style: { backgroundColor: 'red', color: 'white' } }); // Red toast for error
    }
  };
  ...
  return (
    <div className="container mx-auto p-8 max-w-3xl">
      <h2 className="text-4xl font-extrabold text-center text-gray-800 mb-6">To-Do List</h2>
      ...
      >
        {todos.map(todo => (
          <li key={todo._id} className="flex items-center justify-between p-4 bg-white rounded-[4px] border-[1px] border-slate-500 hover:bg-gray-100 transition duration-300 cursor-pointer">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex space-x-4">
                  <FaCheckCircle 
                    ...
                    className={`cursor-pointer ${todo.completed ? 'text-green-500' : 'text-gray-500'} hover:text-green-600 transition duration-200`}
                  />
                  <FaTrash 
